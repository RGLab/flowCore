...
#  - "LargePolygons.xml" Gating-ML file                                            # 
# applied to                                                                       #
...
gateFile <- system.file("unitTests/standardsCompliance/Gating-ML_ComplianceTests/InOutEventsTests/Gating-ML_Files","LargePolygons.xml",package="flowCore")
...
test.G10pts <- function() {
  performGateTest("G10pts", testGates, fcs, expectedResults, numberOfGates)
}

test.G100pts <- function() {
  performGateTest("G100pts", testGates, fcs, expectedResults, numberOfGates)
}

test.G250pts <- function() {
  performGateTest("G250pts", testGates, fcs, expectedResults, numberOfGates)
}

...
gateFile <- system.file("unitTests/standardsCompliance/Gating-ML_ComplianceTests/InOutEventsTests/Gating-ML_Files","LargeEllipsoids.xml",package="flowCore")
...
test.ellipse <- function() {
  performGateTest("ellipse", testGates, fcs, expectedResults, numberOfGates)
}

test.ellipsoid <- function() {
  performGateTest("ellipsoid", testGates, fcs, expectedResults, numberOfGates)
}

test.hyperellipsoid <- function() {
  performGateTest("hyper-ellipsoid", testGates, fcs, expectedResults, numberOfGates)
}

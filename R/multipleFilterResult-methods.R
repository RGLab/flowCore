...
setMethod("summary","multipleFilterResult",function(object,...) {
	...
	structure(list(name=levels(object@subSet),true=true,false=count-true,n=count,p=true/count,q=1-(true/count)),class="filterSummary")
})
...
setMethod("[[",signature("multipleFilterResult"),function(x,i,j,drop=FALSE) {
	...
	new("logicalFilterResult",parameters=x@parameters,subSet=(x@subSet==i & !is.na(x@subSet)),filterDetails=filterDetails,

                    ...
                    , ...)
{
  if(ncdf)
    ...
    mat <- readFCSdata(con, offsets, txt, transformation, which.lines,
                       scale, alter.names, decades, min.limit, truncate_max_range,ignore_TEXT_offsets,debug)
...
readFCSdata <- function(con, offsets, x, transformation, which.lines,
                        scale, alter.names, decades, min.limit=-111, truncate_max_range = TRUE, ignore_TEXT_offsets=FALSE,debug=TRUE) {
    ...
    if(offsets["FCSversion"] >= 3){
        ...
        if(realOff["datastart"] != datastart && realOff["datastart"]!= 0){
			...
			if(ignore_TEXT_offsets){
				warning("The HEADER and the TEXT segment define different ",
						"starting point to read the data. Replace TEXT datastart with HEADER's", call.=FALSE)
				datastart = offsets["datastart"]
			} else {
				stop("The HEADER and the TEXT segment define different ",
						"starting point to read the data. ",
						"Use ignore_TEXT_offsets=TRUE if confidence that HEADERS ",
						"offset is correct. ")
			}
        }
        ...
        if(realOff["dataend"] != dataend && realOff["dataend"]!= 0 && realOff["dataend"]!= 99999999) {
			if(debug){
				print(dataend)
				print(offsets["dataend"])
			}
			if(ignore_TEXT_offsets){
				warning("The HEADER and the TEXT segment define different ",
					"ending point to read the data. Replace TEXT dataend with HEADER's", call.=FALSE)
				dataend = offsets["dataend"]
			} else {
                         ...
                         debug=TRUE, ...)
{
    ...
    flowSet <- lapply(files, read.FCS, alter.names=alter.names,
					  ...
					  ignore_TEXT_offsets=ignore_TEXT_offsets,debug=debug)
